...
- **Pre-built dungeons** (`src/data/PrebuiltDungeons.ts`, GENERATED by
  `tools/dungeons/build_prebuilt.py`, `npm run build-dungeons`): a hundred delves whose floors,
  monster tiles and bosses are fixed data. `src/world/Prebuilt.ts` reads it (`loadPrebuiltFloor`
  paints a floor and returns its rooms; `pickPrebuilt` assigns one per overworld gate, theme
  first). Every entrance carries `prebuiltId`; `Game.currentPrebuilt()` finds it from
  `dungeonEntranceId`, `generateNewDungeon` loads the floor instead of generating when one
  exists, and `populatePrebuiltFloor` spawns the listed creatures, the hunt quarry if a posting
  needs it, and the boss through `spawnFloorBoss(hall, fixed)` so the story's antagonist still
  overrides on its act's floor. The bottom floor has no stairs, so the party climbs out. The
